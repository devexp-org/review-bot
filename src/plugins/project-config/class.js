...
export default class ProjectConfig {
  ...
  constrcutor(logger, github, teamDispatcher, options) {
    this.looger = logger;
    this.github = github;
  ...
  process(pullRequest) {
    return this.readConfig(pullRequest)
      .then(patch => {
        ...
        return [].concat(promoted).concat(excluded);
      });
  }
  ...
  _promoteReviewers(toPromote) {
    return toPromote
      .map(user => {
        return { special: true, login: user.login, rank: 1000 };
      });
  }
  ...
  _excludeReviewers(toRemove) {
    return toRemove
      .map(user => {
        return { login: user.login, rank: -Infinity };
      });
  }
  ...
  _applyRules(pullRequest, config) {
    ...
    _.forEach(config.specialReviewers, (rule) => {
      ...
      _.forEach(rule.pattern, (pattern) => {
        ...
        if (this._matchSome(files, pattern)) {
          ...
          const addMembers = _.sample(rule.addMembers, userCount)
    ...
    this._filterMembers(pullRequest, patch);
